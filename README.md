...
To Configure **Patient identifier creation**, set the `fetch_identifier` property to either of the following:
1. `true` : if you want to fetch (auto generated) identifier's value on android app and embed to the patient before uploading to server.
2. `false`: if you don't want to fetch (auto generated) identifier's value on android app and embed to the patient before uploading to server. (essentially handling their creation on backend or you don't use autogenerated identifiers).
